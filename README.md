# LED Matrix Painter ComposeApp

This repository serves as an example of how to bundle the
[LED Matrix Painter](https://github.com/arduino/app-bricks-examples/tree/release-0.6/examples/led-matrix-painter)
...
```
 ...
 $ git submodule add -b arduino-apps/led-matrix-painter https://github.com/foundriesio/containers led-matrix-painter
```

This example makes use of the [web_ui](https://github.com/arduino/app-bricks-py/blob/release/0.6.4/src/arduino/app_bricks/web_ui)
...
Each application includes an [app.yaml](https://github.com/arduino/app-bricks-examples/blob/0.6.3/examples/led-matrix-painter/app.yaml)
 ...
 * `sketch` - Similar to the `py` folder, this builds a container to
   flash the sketch.
 * `.ci-search-dirs` - A file that tells the Foundries CI system to look
   one level of directories down to find Dockerfiles to build. In this case, it will find the `py` directory and build a container, `led-matrix-painter-py` as well as `led-matrix-painter-sketch`.

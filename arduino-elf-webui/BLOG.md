# Building an Object Detection System with YOLOv5 on Arduino Uno Q

## Introduction

In our previous tutorials, we explored the Arduino Uno Q's dual-core architecture through [web-controlled LED systems](../arduino-led-webui/BLOG.md) and [voice-controlled Christmas tree](../arduino-voice-webui/BLOG.md). Now, let's take edge AI to the next level by building a **real-time object detection system** using computer vision.

This project leverages **Edge Impulse** and **YOLOv5**, a state-of-the-art object detection model, to recognize objects directly on the Arduino Uno Q's Linux core (MPU). No cloud connectivity required - everything runs locally on the device!

We'll build an application where you can:
- Point a USB camera at objects
- Watch as the system detects and labels them in real-time
- See **bounding boxes** with confidence scores overlaid on the video
- Run inference at ~17 FPS (~58ms per frame) on the edge device

---

## Why Edge AI for Object Detection?
...
With **Edge Impulse + YOLOv5** running on the Arduino Uno Q, we process everything locally:

```
Camera → Arduino MPU → Object Detection → Immediate Display
       (no internet needed!)
```
...
Building on our [previous LED controller tutorial](../arduino-led-webui/BLOG.md), we're using the Arduino Uno Q's **MPU (Linux core)** to run the object detection system:
...
To build our object detection model, we followed these steps:

#### 1. Dataset Creation

Classes defined:
- **elf** - Christmas elf figurines
- **person** - Human faces/bodies
...
- 50+ images of "elf" from different angles
- 50+ images of "person" in various poses
...
Let's examine the `Dockerfile` to understand how we set up the Edge Impulse + OpenCV environment:
...
1. **Open browser**: Navigate to `http://<arduino-ip>:8000`
2. **Point camera**: Aim at objects (elf, person)
3. **Observe detections**: Bounding boxes should appear
4. **Check confidence**: Color indicates detection quality
